...
namespace StudentManagemetSystem.Helpers
{
    public class ConsoleIO
    {
        ...
        public static string GetRequiredStringFromUser(string prompt)
        {
            while (true)
            {
                ...
                if (string.IsNullOrEmpty(input)) //input validation
                {
        ...
        public static string GetYesNoAnswerFromUser(string prompt)
        {
            while (true)
            {
                ...
                {
                    if (input != "Y" && input != "N") //input validation
                    {
                        Console.WriteLine("You must enter Y/N.");
                        Console.WriteLine("Press any key to continue...");
                        Console.ReadKey();
        ...
        public static decimal GetRequiredDecimalFromUser(string prompt)
        {
            ...
            while (true)
            {
                ...
                if (!decimal.TryParse(input, out output)) //input validation
                {
                ...
                {
                    if (output < 0 || output > 4) //check to make sure GPA is between 0 and 4.0
                    {
        ...
        public static int GetStudentIndexFromUser(string prompt, int count)
        {
            ...
            while (true)
            {
                ...
                if (!int.TryParse(input, out output)) //input validation
                {
                ...
                {
                    if (output < 1 || output > count) //check to make sure GPA is between 0 and 4.0
                    {
